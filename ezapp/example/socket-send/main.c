...
{
    rotate_send_buffer();
    return ezbus_socket_send( socket, feature_state.send_buffer, AFX_MAX_PACKET_SZ ) > 0;
}

extern bool ezbus_socket_callback_recv ( ezbus_socket_t socket )
{
    int bytes_recvd;
    if ( (bytes_recvd=ezbus_socket_recv( socket, feature_state.recv_buffer, AFX_MAX_PACKET_SZ )) > 0 )
    {
