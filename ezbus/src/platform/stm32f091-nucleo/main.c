...
char ezbus_thread_stack[THREAD_STACK_SIZE];			// Stack space for the ezbus thread.
...
void button_thread(void* arg)
{
	for(;;) // forever...
	{
		char* msg = (char*)caribou_malloc(2);				// get a 2 byte message buffer from the heap.
		caribou_thread_sleep_current(from_ms(10));			// check the button every 10 ms
		if( !caribou_gpio_pinstate(&pb1) )
			caribou_strcpy(msg,"D");						// make a message to indicate button down.
		else
			caribou_strcpy(msg,"U");						// make a message to indicate button up.
...
void led_thread(void* arg)
{
	...
	{
		char* msg=NULL;
        if ( caribou_queue_take_first(&queue,(void**)&msg,TIMEOUT_INFINITE) )
		{
			if ( caribou_strcmp(msg,"D") )				// Button Pressed message?
			{
			...
			caribou_free(msg);							// Free the message buffer
		}
	}
}
...
int main(int argc,char* argv[])
{
	...
	caribou_queue_init(&queue,QUEUE_DEPTH,(void**)&queue_msgs);

    /** Allocate and start up the enqueue and dequeue threads... */
	caribou_thread_create("button",button_thread,NULL,NULL,button_thread_stack,THREAD_STACK_SIZE,THREAD_PRIORITY);
	caribou_thread_create("led",led_thread,NULL,NULL,led_thread_stack,THREAD_STACK_SIZE,THREAD_PRIORITY);

	/** Allocate and start up the ezbus thread... */
	caribou_thread_create("ezbus",ezbus_thread,NULL,NULL,ezbus_thread_stack,THREAD_STACK_SIZE,THREAD_PRIORITY);
